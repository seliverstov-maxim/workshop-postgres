...
* 
__Тривиальная и не тривиальная форма__:
Тривиальная форма зависимости это зависимость в которой зависимая часть является подмножеством детеминанта. Например: `{name, surname, course} → {course}` или `{name, surname, course} → {name}`

## Многозначная зависимость

__многозначная зависимость__ - это зависимось в которой для каждого детерминанта существует множество зависимостей. Записывается как X →→ Y. Наприер `{Автор} →→ {Книга}`.

Автор|Книга
---|---
Толстой|Анна Коренина
Толстой|Война и мир
Толстой|Детство
Толстой|Отрочество
Гоголь|Мертвые души
